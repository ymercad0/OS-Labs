...
int main(int argc, char * argv[]) {
    ...
    printf("pi = %.15f\n", result);  // 10000 iterations and 4 threads: 3.141692643590569
}
...
void *runner(void * param) {
    ...
    //complete function
    // Each thread will execute for #iterations/#thread times. This is why we need to increase n by #threads times.
    for (int n = threadid-1; n <= iterations; n+=threads){
        // Summing all the terms in one spot for that thread's cycles.
        // In the end, it doesn't matter in what order we sum the terms.
